...
extends MixinRenderer<EntityItem> {
    ...
    private int transformModelCount(EntityItem itemIn, double x, double y, double z, float p_177077_8_, IBakedModel p_177077_9_) {
        if (_as_0.fB.getModule("ItemPhysics").isEnabled()) {
            ...
            if ((itemstack = (entityitem = entity).getItem()).getItem() != null) {
                ...
                for (int j = 0; j < i; ++j) {
                    if (ibakedmodel.isAmbientOcclusion()) {
                        GlStateManager.pushMatrix();
                        GlStateManager.scale((double)_cS.sH.getDoubleValue(), (double)_cS.sH.getDoubleValue(), (double)_cS.sH.getDoubleValue());
                        if (_as_0.fB.getModule("ESP").isEnabled() && _bG.mD.isEnabled()) {
                            ...
                            _bi_0.renderThree();
                            this.itemRenderer.renderItem(itemstack, ibakedmodel);
                            _bi_0.renderFour(_bG.mE.getColor());
                            this.itemRenderer.renderItem(itemstack, ibakedmodel);
                            _bi_0.renderFive();
                            _bi_0.setColor(_bG.mE.getColor());
                        } else {
                            this.itemRenderer.renderItem(itemstack, ibakedmodel);
                        }
